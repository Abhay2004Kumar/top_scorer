...
const BlogFeed = () => {
  ...
  const sendComment = async () => {
    ...
    try {
      await axios.post(
        `${process.env.REACT_APP_BACKEND_URL}/api/v1/users/commentBlog`,
        {
          blogId: selectedBlog._id,
          content: comment,
        }
      ...
      {isModalOpen && selectedBlog && (
        <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg w-full max-w-2xl md:max-w-4xl relative flex flex-col md:flex-row gap-6 max-h-[80vh] overflow-hidden">
            <button
              className="absolute top-2 right-4 text-gray-700 dark:text-white"
              onClick={closeModal}
            >
              X
            </button>
            ...
            <div className="w-full md:w-2/3 flex flex-col overflow-y-auto">
              ...
              <img
                ...
                className="w-full h-32 md:h-64 object-cover mt-4 rounded-md"
              />
              <p dangerouslySetInnerHTML={{__html:selectedBlog.content}} className="text-gray-700 dark:text-gray-300 mt-4">
              ...
              <div className="mt-4 flex items-center gap-4">
                <button
                  className="bg-blue-500 text-white px-4 py-2 rounded-full flex items-center gap-1"
                  onClick={doLike}
                >
